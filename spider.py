...
path = os.path.dirname(__file__)  # 获取当前python文件所在目录的完整路径
data_path = os.path.join(path, "tempdata")  # 获取的数据保存在temp目录中
if not os.path.exists(data_path):
    os.mkdir(data_path)  # 如果tempdata目录不存在，则创建该目录

f = open(os.path.join(data_path, 'data.txt'), 'w', encoding='UTF-8', errors='ignore') 
f.write('日期      质量等级      AQI\n')
...
ses.get("http://www.tianqihoubao.com/aqi/dongguang-201501.html", headers=request_headers)
